...
results <- data.frame(simno = 1:Nsim, ind.estimate = numeric(Nsim), ind.se = numeric(Nsim), ind.sesq = numeric(Nsim), ind.pval = numeric(Nsim),
                      ...
                      total = numeric(Nsim), covariance = numeric(Nsim), total_se = numeric(Nsim), total_sesq = numeric(Nsim),
...
for (i in 1:Nsim){
    ...
    tryCatch(
      {
        ...
        dff$timept <- as.factor(dff$timept)   # Time is treated as discrete
    ...
    tryCatch(
      {
        ...
        # Ind time is treated as discrete; others already set as discrete
        ind.time <- dff$ord - 1
  ...
  if(!is.na(results$estimate[i])){
    ...
    results$estimate[i] <- mat$coef.value[mat$coef.name=="timept78:trt1"] 
    results$se[i] <- mat$coef.se[mat$coef.name=="timept78:trt1"] 
    results$sesq[i] <- (results$se[i])^2
    results$pval[i] <- mat$coef.pval[mat$coef.name=="timept78:trt1"] 
  }
  ...
  if((scenarios$design[k]=="oc" ) | (scenarios$design[k]=="cs")){ 
    if(!is.na(results$ch.estimate[i])){
      ...
      results$ind.estimate[i] <- ifelse(length(mat_lem$coef.value[mat_lem$coef.name=="ind.time78:trt1"])!=0, mat_lem$coef.value[mat_lem$coef.name=="ind.time78:trt1"],NA)
      results$ind.se[i] <- ifelse(length(mat_lem$coef.se[mat_lem$coef.name=="ind.time78:trt1"])!=0, mat_lem$coef.se[mat_lem$coef.name=="ind.time78:trt1"],NA) 
      results$ind.sesq[i] <- (results$ind.se[i])^2
      results$ind.pval[i] <- ifelse(length(mat_lem$coef.pval[mat_lem$coef.name=="ind.time78:trt1"])!=0, mat_lem$coef.pval[mat_lem$coef.name=="ind.time78:trt1"],NA) 
      results$ch.estimate[i] <- ifelse(length(mat_lem$coef.value[mat_lem$coef.name=="timept78:trt1"])!=0, mat_lem$coef.value[mat_lem$coef.name=="timept78:trt1"],NA) 
      results$ch.se[i] <- ifelse(length(mat_lem$coef.se[mat_lem$coef.name=="timept78:trt1"])!=0, mat_lem$coef.se[mat_lem$coef.name=="timept78:trt1"],NA) 
      results$ch.sesq[i] <- (results$ch.se[i])^2
      results$ch.pval[i] <- ifelse(length(mat_lem$coef.pval[mat_lem$coef.name=="timept78:trt1"])!=0, mat_lem$coef.pval[mat_lem$coef.name=="timept78:trt1"],NA) 
      results$total[i] <- sum(results$ind.estimate[i],results$ch.estimate[i],na.rm=TRUE)
      results$covariance[i] <- tryCatch({vcov(sum_lem)["timept78:trt1","ind.time78:trt1"]; vcov(sum_lem)["timept78:trt1","ind.time78:trt1"]}, error=function(e) NA)
...
# Average estimates  
finalresults$avg_ind_estimate <- mean(results$ind.estimate[!is.na(results$total)])  # The is.na is for the total estimate - ONLY FOR DISCRETE ESTIMAND
finalresults$avg_ch_estimate <- mean(results$ch.estimate[!is.na(results$total)]) # The is.na is for the total estimate - ONLY FOR DISCRETE ESTIMAND
finalresults$avg_total_estimate <-  mean(results$total[!is.na(results$total)])

# Power (not used)
finalresults$ind_power <- sum(results$ind.pval[!is.na(results$total)]<0.05)/(Nsim-singul2)
finalresults$ch_power <- sum(results$ch.pval[!is.na(results$total)]<0.05)/(Nsim-singul2)

# Average of model based SE's (not used)
finalresults$avg_ind_se <- mean(results$ind.se[!is.na(results$total)])
finalresults$avg_ind_se2 <- sqrt(mean(results$ind.sesq[!is.na(results$total)]))
finalresults$avg_ch_se <- mean(results$ch.se[!is.na(results$total)])
finalresults$avg_ch_se2 <- sqrt(mean(results$ch.sesq[!is.na(results$total)]))
finalresults$avg_total_se <-  mean(results$total_se[!is.na(results$total)])
finalresults$avg_total_se2 <-  sqrt(mean(results$total_sesq[!is.na(results$total)]))

# Empirical SE's
finalresults$ind_emp_se <- sqrt(1/(Nsim-singul2-1)*sum((results$ind.estimate[!is.na(results$total)]-finalresults$avg_ind_estimate)^2))
finalresults$ch_emp_se <- sqrt(1/(Nsim-singul2-1)*sum((results$total[!is.na(results$total)]-finalresults$avg_ch_estimate)^2))
finalresults$total_emp_se <- sqrt(1/(Nsim-singul2-1)*sum((results$total[!is.na(results$total)]-finalresults$avg_total_estimate)^2)) 
...
#############################################################
saveRDS(finalresults, file=paste("discretefinalresults",k,".Rds",sep=""))
